...
impl ServerCluster {
    ...
    fn start_server(&self, name: &str, instance_config: &FilledInstanceConfig, config: &Config, install_config: &InstallConfig) -> Result<RunningServer, Box<dyn Error>> {
        ...
        let child = Command::new(&install_config.full_exe_path)
            .current_dir(&install_config.game_dir)
            .stdout(Stdio::inherit())
            .stderr(Stdio::inherit())
            .spawn()?;

        info!("Server {} has started with process {}", name, child.id());
        let process = Process::new(child.id()).ok_or(StartInstanceError::ProcessCrashedWhileStarting)?;
        loop {
