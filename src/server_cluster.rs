...
use crate::protocol::{receive_connect_reply, send_connect};
use crate::Config;
...
impl Display for StartServerError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            StartServerError::ContainerCrashedWhileStarting => {
                write!(f, "The container crashed while initializing")
            }
            StartServerError::ContainerHasNoIp => {
                write!(f, "The container was not assigned an IP address")
            }
...
impl Server {
    ...
    pub async fn start(&mut self, game_port: u16, docker: &Docker) -> Result<(), Box<dyn Error>> {
        ...
        let container_game_address = SocketAddrV4::new(container_ip, game_port);
